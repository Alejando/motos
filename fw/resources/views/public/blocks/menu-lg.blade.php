...
?><ul class="menumain hidden-sm hidden-xs">
    @foreach($categories as $category)
        ...
        @if($subs->count())
            ...
            <li>
                <a href="{{$category->getURL()}}" class="transicion">{{strtoupper($category->name)}}</a>
                        ...
                        @foreach($subs as $sub)
                            @if($sub->subcategories->count())
                                <div style="width:{{$width}}em; float:left;">
                                    <h3><span><a href="{{$sub->getURL()}}">{{$sub->name}}</a></span> <i class="fa fa-angle-down" aria-hidden="true"></i></h3>
                                </div>
                            @else
                                <div style="width:{{$width}}em; float:left;">
